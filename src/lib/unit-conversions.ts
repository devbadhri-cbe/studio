 ...
 */
export function toMmolL(value: number, type: 'ldl' | 'hdl' | 'total' | 'triglycerides'): number {
 ...
 */
export function toMgDl(value: number, type: 'ldl' | 'hdl' | 'total' | 'triglycerides'): number {
  if (type === 'triglycerides') {
    return value / LIPID_CONVERSION_FACTOR_TRIG;
  }
  return value / LIPID_CONVERSION_FACTOR_LDL_HDL_TOTAL;
