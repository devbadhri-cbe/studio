...
export function HdlCard({ isReadOnly = false }: HdlCardProps) {
  ...
  const sortedRecords = React.useMemo(() => {
    return [...(hdlRecords || [])].sort((a,b) => new Date(a.date as string).getTime() - new Date(b.date as string).getTime())
  }, [hdlRecords]);
  
  const getStatus = (value: number) => {
    const isMale = profile.gender === 'male';
    if ((isMale && value < 40) || (!isMale && value < 50)) return { text: 'Low', variant: 'destructive' as const };
    if (value > 60) return { text: 'High (Optimal)', variant: 'outline' as const };
  ...
  const Title = `HDL Cholesterol (${unitLabel})`;
              ...
              <li key={record.id} className="group flex items-center gap-2 text-xs text-muted-foreground border-l-2 border-primary pl-3 pr-2 py-1 hover:bg-muted/50 rounded-r-md">
                  ...
                  {!isReadOnly && (
                    <Tooltip>
                        ...
                        <Button size="icon" variant="ghost" className="h-5 w-5 shrink-0 opacity-0 group-hover:opacity-100" onClick={() => removeHdlRecord(record.id)}>
  ...
  return (
    <BiomarkerCardTemplate
      ...
      chart={Chart}
      hasRecords={(hdlRecords || []).length > 0}
