...
export function HemoglobinChart() {
  const { hemoglobinRecords, profile, biomarkerUnit, getDisplayHemoglobinValue } = useApp();
  const formatDate = useDateFormatter();
  const unitLabel = biomarkerUnit === 'si' ? 'g/L' : 'g/dL';

  const sortedRecords = [...(hemoglobinRecords || [])].sort((a,b) => new Date(a.date as string).getTime() - new Date(b.date as string).getTime());
    ...
    <div className="h-full w-full flex flex-col">
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          {hemoglobinRecords && hemoglobinRecords.length > 0 ? (
