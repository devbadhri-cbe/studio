...
interface CustomBiomarkerChartProps {
    biomarkerId: string;
}

export function CustomBiomarkerChart({ biomarkerId }: CustomBiomarkerChartProps) {
  const { profile } = useApp();
  const formatDate = useDateFormatter();
  
  const records = profile.customBiomarkerRecords?.[biomarkerId] || [];

  const sortedRecords = [...records].sort((a,b) => new Date(a.date as string).getTime() - new Date(b.date as string).getTime());
        ...
        <ResponsiveContainer width="100%" height="100%">
          {records.length > 0 ? (
            <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 20, left: 0 }}>
              ...
              />
              <Line type="monotone" dataKey="value" name="Value" stroke="hsl(var(--chart-1))" strokeWidth={2} dot={<Dot r={4} fill="hsl(var(--chart-1))" />} activeDot={{ r: 6 }} />
            </LineChart>
          ) : (
