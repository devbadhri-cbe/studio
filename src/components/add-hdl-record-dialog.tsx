...
const FormSchema = z.object({
  ...
  value: z.coerce.number().min(1, 'Value seems too low.').max(200, 'Value seems too high.'),
});
...
export function AddHdlRecordDialog({ children }: AddHdlRecordDialogProps) {
  ...
  const onSubmit = (data: z.infer<typeof FormSchema>) => {
   ...
   const triggerButton = children || (
      <AddRecordButton tooltipContent="Add HDL Record" />
   );
  ...
  return (
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Add New HDL Cholesterol Record</DialogTitle>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
              ...
              <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>HDL Cholesterol ({unitLabel})</FormLabel>
                      <FormControl>
