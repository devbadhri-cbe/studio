...
const FormSchema = z.object({
  ...
  value: z.coerce.number().min(1, 'Value seems too low.').max(5000, 'Value seems too high.'),
});
...
export function AddTriglyceridesRecordDialog({ children }: AddTriglyceridesRecordDialogProps) {
  const [open, setOpen] = React.useState(false);
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const { addTriglyceridesRecord } = useApp();
  ...
  return (
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Add New Triglycerides Record</DialogTitle>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
              ...
              <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Triglycerides ({unitLabel})</FormLabel>
                      <FormControl>
