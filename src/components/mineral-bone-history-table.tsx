...
import type { MineralBoneDiseaseRecord } from '@/lib/types';
import { Tooltip, TooltipContent, TooltipTrigger } from './ui/tooltip';
import { useDateFormatter } from '@/hooks/use-date-formatter';

const RECORDS_PER_PAGE = 5;

export function MineralBoneHistoryTable() {
  const { mineralBoneDiseaseRecords, removeMineralBoneDiseaseRecord } = useApp();
  const [currentPage, setCurrentPage] = React.useState(1);
  const [selectedRecord, setSelectedRecord] = React.useState<MineralBoneDiseaseRecord | null>(null);
  const formatDate = useDateFormatter();

  const sortedRecords = React.useMemo(() => {
    return [...(mineralBoneDiseaseRecords || [])].sort((a, b) => new Date(a.date as string).getTime() - new Date(b.date as string).getTime());
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="px-2 md:px-4">Calcium (mg/dL)</TableHead>
              <TableHead className="px-2 md:px-4">Phosphorus (mg/dL)</TableHead>
              <TableHead className="px-2 md:px-4">PTH (pg/mL)</TableHead>
                ...
                paginatedRecords.map((record) => {
                  return (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="px-2 md:px-4">{record.calcium.toFixed(1)}</TableCell>
                      <TableCell className="px-2 md:px-4">{record.phosphorus.toFixed(1)}</TableCell>
                      <TableCell className="px-2 md:px-4">{record.pth}</TableCell>
                      <TableCell className="text-right px-2 md:px-4">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onSelect={() => removeMineralBoneDiseaseRecord(record.id)}>Delete</DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </TableCell>
                    </TableRow>
